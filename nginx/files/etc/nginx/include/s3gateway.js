...
var metadataHostIsSet = "METADATA_ENDPOINT" in process.env;
if (!metadataHostIsSet) {
...
var resizeLambda = process.env['RESIZE_LAMBDA'] || false;
var resizeRegexp = new RegExp(process.env.RESIZE_REGEX || "^.", 'i');
...
const redirecToIndex = _parseBoolean(process.env['DIRECTORY_TO_INDEX'] || false);
 ...
 */
async function s3auth(r) {
    var metadataHost = process.env['METADATA_ENDPOINT'] || false;
    let accessId;
    let secret;
    if (metadataHost) {
        _debug_log(r, `http://${metadataHost}/latest/api/token - Getting Access token from metadata endpoint`);
        var response = await ngx.fetch(`http://${metadataHost}/latest/api/token`, {
            headers: {"X-aws-ec2-metadata-token-ttl-seconds": 600},
            method: "PUT"
        })
            .then(response => response.text())
            .then((token) => {
                _debug_log(r, "Getting s3 access token");
                return ngx.fetch(`http://${metadataHost}/latest/meta-data/iam/security-credentials/s3access`, {headers: {"X-aws-ec2-metadata-token": token}})
            })
    ...
    r.status = 204;
    r.headersOut['s3_auth'] = signature;
    r.sendHeader();
    r.finish();
}
...
function s3uri(r) {
    ...
    if (allow_listing) {
        ...
        if (_isDirectory(uriPath) && redirecToIndex) {
            path = basePath + uriPath + 'index.html';
        } else {
            path = basePath + uriPath;
        }
    }
...
function _s3DirQueryParams(uriPath, method) {
    if (!_isDirectory(uriPath) || method !== 'GET' || redirecToIndex) {
        return '';
    }
...
function redirectToS3(r) {
    // This is a read-only S3 gate*way, so we do not support any other methods
...
/**
 * Handles the error depending on different environment variables.
 * if REDIRECT_LAMBDA is set, it will redirect to the specified function, with the key being requested as part of the arguments.
 * 
 * @param r {Request} HTTP request object
 */
function handleError(r) {
    ...
    if(resizeLambda && resizeRegexp.test(uriPath)) {
        _debug_log(r,"Redirecting to lambda: " + uriPath);
        r.return(307,[resizeLambda,"?",querystring.stringify({key: uriPath})].join(""));
    } else {
        _debug_log(r,"Not redirecting: " + uriPath + " Regexp: " + resizeRegexp);
...
function _buildSignatureV4(r, amzDatetime, eightDigitDate, bucket, secret, region, server) {
    ...
    _debug_log(r, 'AWS v4 Auth Canonical Request: [' + JSON.stringify(canonicalRequest) + ']');
    ...
    if ("variables" in r && r.variables.cache_signing_key_enabled == 1) {
        ...
        if (cacheIsValid) {
            ...
            kSigningHash = _buildSigningKeyHash(secret, eightDigitDate, service, region,r);
