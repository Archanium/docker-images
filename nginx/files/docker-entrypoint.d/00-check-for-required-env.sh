...
if [ -z ${METADATA_ENDPOINT+x} ]; then
  if [ -z ${S3_ACCESS_KEY_ID+x} ]; then
    >&2 echo "Required S3_ACCESS_KEY_ID or METADATA_ENDPOINT environment variable missing"
    failed=1
  fi

  if [ -z ${S3_SECRET_KEY+x} ]; then
    >&2 echo "Required S3_SECRET_KEY or METADATA_ENDPOINT environment variable missing"
    failed=1
  fi
fi
