...
RUN apt-get update && apt-get install -y libz-dev libmemcached-dev && pecl install memcached && docker-php-ext-enable memcached && apt-get clean
...
RUN curl -L -o /usr/local/bin/phan https://github.com/phan/phan/releases/download/5.3.1/phan.phar && chmod +x /usr/local/bin/phan
RUN curl -L -o /usr/local/bin/composer https://github.com/composer/composer/releases/download/2.1.6/composer.phar && chmod +x /usr/local/bin/composer
WORKDIR /mnt/src
